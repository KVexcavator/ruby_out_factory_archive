# encoding: cp1251

print "‘Є®«мЄ® ў ¬ «Ґв? "
age = gets.to_i

print "•®вЁвҐ ЁЈа вм? (Y/N) "
answer = gets.strip.capitalize

if answer == "Y" && age >= 18
	puts "•®а®и®, Ї®ЁЈа Ґ¬!"

	money = 100
	
	add_money_proc = lambda { |sum, money|
		puts "‚ ¬ § зЁб«Ґ­® #{sum} ¤®«« а®ў"
		money + sum
	}
	
	sub_money_proc = lambda { |sum, money|
		puts "‚л Ї®вҐап«Ё #{sum} ¤®«« а®ў"
		money - sum
	}
	
	actions =
	{
		"000" => lambda { |money|
				puts "‚ и Ў « ­б ®Ў­г«Ґ­!"
				0
		},
		"111" => [10, add_money_proc],
		"222" => [20, add_money_proc],
		"333" => [30, add_money_proc],
		"444" => [40, add_money_proc],
		"555" => [50, add_money_proc],
		"666" => lambda { |money|
				puts "‚л Ї®вҐап«Ё Ї®«®ўЁ­г бг¬¬л"
				money / 2
		},
		"777" => [70, sub_money_proc],
		"888" => [80, sub_money_proc],
		"999" => [90, sub_money_proc],
		"123" => [123, sub_money_proc],
	}

	1000.times do

		puts "‚ўҐ¤ЁвҐ Enter зв®Ўл ¤Ґа­гвм агзЄг"
		gets

		num = rand(0..9).to_s + rand(0..9).to_s + rand(0..9).to_s

		puts "‚лЇ «® #{num}"
		
		if actions.has_key?(num)
			if actions[num].is_a?(Array)
				money = actions[num][1].call(actions[num][0], money)
			elsif actions[num].is_a?(Proc)
				money = actions[num].call(money)
			end
		end

		puts "Ћбв «®бм ¤Ґ­ҐЈ: #{money} ¤®«« а®ў"
		
		if money == 0
			puts "“ ў б ­Ґ ®бв «®бм ¤Ґ­ҐЈ"
			break
		elsif money < 0
			puts "‚л Їа®¤ «Ё ўбс бў®Ґ Ё¬гйҐбвў®, зв®Ўл а бЇ« вЁвмбп Ё Ї®Є®­зЁ«Ё б б®Ў®©"
		end
	end
end
 